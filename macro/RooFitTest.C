...
//  int i=4;
  for(int i=0; i<m2p->GetXaxis()->GetNbins(); ++i)
  {
    ...
    std::cout << "Momentum - " << m2p->GetXaxis()->GetBinCenter(i+1) << std::endl;
    ...
    RooRealVar pi_frac("pi_frac","pi_frac", 0.7, 0., pi_frac_max);
    RooRealVar K_frac("K_frac","K_frac", 0.03, 0.0, K_frac_max);
    RooAddPdf sum("sum","0.01 + pi+K+p",RooArgList(pi, K, p), RooArgList(pi_frac, K_frac));

    RooDataHist data("data","data", RooArgList(m2), Import(*slice));
