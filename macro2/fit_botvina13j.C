...
void fit_botvina13j () {
  ...
  TString inputFileName = "../input/fVirgoImpact.root";
  TString cutsFileName = "../input/cuts2.root";
  ...
  int k=0;
  TString kString = (Form("%d",k));
  cout << "\n\npionpos\n";
  xmin = 0.25, xmax = 12., ymin = -6., ymax = 6.;
  tof.SetChi2Max(10000);
  std::unique_ptr <TH2D> hpionpos {(TH2D*) fIn->Get("reco_vs_sim_info"+kString+"to"+kString+"/histoMass2vtxPi")};
  hpionpos->Rebin2D(rebin);
  ...
  */
  ...
  cout << "\n\nkaonpos\n";
  xmin = 0.4, xmax = 12., ymin = -6, ymax = 6.;
  tof.SetChi2Max(1000);
  std::unique_ptr <TH2D> hkaonpos {(TH2D*) fIn->Get("reco_vs_sim_info"+kString+"to"+kString+"/histoMass2vtxKa")};
  hkaonpos->Rebin2D(rebin);
  ...
  */
  ...
  tof.SetChi2Max(2000);
  std::unique_ptr <TH2D> hproton {(TH2D*) fIn->Get("reco_vs_sim_info"+kString+"to"+kString+"/histoMass2vtx")};
  hproton->Rebin2D(rebin);
  std::unique_ptr <TH2D> hproton_cut {(TH2D*) cutTH2 (hproton, (TCutG*) fCuts->Get("proton"))};
  hproton_cut->Rebin2D(10,1);
  ...
  */
  ...
  cout << "\n\nhe3\n";
  std::unique_ptr <TH2D> hhe3 {(TH2D*) fIn->Get("reco_vs_sim_info"+kString+"to"+kString+"/histoMass2vtxh3")};
  hhe3->Rebin2D(rebin);
  std::unique_ptr <TH2D> hhe3_cut {(TH2D*) cutTH2 (hhe3, (TCutG*) fCuts->Get("he3"))};
  hhe3_cut->Rebin2D(10,1);
  xmin = 1.8, xmax = 8.7, ymin = -6., ymax = 6.;
  tof.SetChi2Max(10);
  ...
  */
  ...
  TF1 he3_2 ("he3_2", "pol10", xmin, xmax);
  ...
  cout << "\n\ndeutron\n";
  xmin = 1.3, xmax = 12., ymin = -6., ymax = 6.;
  tof.SetChi2Max(1000);
  std::unique_ptr <TH2D> hdeutron {(TH2D*) fIn->Get("reco_vs_sim_info"+kString+"to"+kString+"/histoMass2vtxDe")};
  hdeutron->Rebin2D(rebin);
  std::unique_ptr <TH2D> hdeutron_cut {(TH2D*) cutTH2 (hdeutron, (TCutG*) fCuts->Get("deutron"))};
  hdeutron_cut->Rebin2D(10,1);
...
*/
  ...
  cout << "\n\nallpos\n";
  xmin = 0.3, xmax = 20., ymin = -6., ymax = 6.;
  std::unique_ptr <TH2D> hpos {(TH2D*) fIn->Get("reco_info"+kString+"to"+kString+"/histoMass2")};
  hpos->Rebin2D(rebin);
  hpos->Rebin2D(10,1);
  tof.SetChi2Max(10000);
