...
*/
void Fitter::Fit() {
  const auto firstbin = histo2D_->GetXaxis()->FindBin(minx_);
  const auto lastbin = histo2D_->GetXaxis()->FindBin(maxx_);
  ...
  for (int ibin = firstbin; ibin < lastbin; ++ibin) {
    ...
    const auto mom = histo2D_->GetXaxis()->GetBinCenter(ibin);
    const auto chi2 = Fit1D(h1fit, par, par_err, mom);
