...
class ParticlePdf2DBase{
 public:
  ...
  virtual ParticlePdf1DBase* GetPdf1D(float x) const = 0;
...
class ParticlePdf2DBinned : public ParticlePdf2DBase {
 public:
  ...
  float GetPdfValue(float x, float y) const override {
    ...
    return bin>=0 && bin<pdfs_.size() ? pdfs_.at(FindBin(x))->GetPdfValue(y) : 0.f;
  }

  ParticlePdf1DBase* GetPdf1D(float x) const override {
    const int bin = FindBin(x);
